...
class MergerTree:
    ...
    def load_halo(self, pdir, sub_tree, redshift=None, snapshot=None):
        ...
        rad = (halo['virial_radius'].values[0].astype(float) , 'kpccm')
        cen = ds.arr(halo[['position_x', 'position_y', 'position_z']].values[0].astype(float), 'kpccm')
        
        sphere = ds.sphere(cen, rad)
