...
class SnapshotHalo(BaseHaloObject):
    ...
    **kwargs: dictionary of keyword arguments that can be passed when loading the halo. They are the following:
    ...
    def __init__(self,
        ...
        self._sha = {
            ("halo", "rck_center"): None,
            ("halo", "rck_velocity"): None,
            ("halo", "rck_rs"): None,
            ("halo", "rck_rvir"): None,
            ("halo", "rck_vmax"): None,
            ("halo", "rck_vrms"): None,
    ...
    def _setup_sha(self):
        ...
        """
        fields = {
             ("halo","rck_center"):    self.arr(self._cathalo[['position_x', 'position_y', 'position_z']].values.astype(float), 'kpccm').to("kpc"),
             ("halo","rck_velocity"):  self.arr(self._cathalo[['velocity_x', 'velocity_y', 'velocity_z']].values.astype(float), 'km/s'),
             ("halo","rck_rs"):        self.quant(self._cathalo['scale_radius'], 'kpccm').to("kpc"),
             ("halo","rck_rvir"):      self.quant(self._cathalo['virial_radius'], 'kpccm').to("kpc"),
             ("halo","rck_vmax"):      self.quant(self._cathalo['vmax'], 'km/s'),
             ("halo","rck_vrms"):      self.quant(self._cathalo['vrms'], 'km/s')
        }
    ...
    def set_line_of_sight(self, los):
        ...
        for q in ["cm", "vcm", "rockstar_velocity", "rockstar_center"]:
            self.sha = {("halo", q) : self._old_to_new_base @ self[q] if self[q] is not None else None }
    ...
    def compute_energy(self,  
        ...
        for i in range(100):
            ...
            if (delta_cm and delta_vcm):
                self.sha = {("halo", "cm"): new_cm}
                self.sha = {("halo", "vcm"): new_cm}
    ...
    def compute_stars_in_halo(self,
        ...
        halo_params = {
            'center': self["halo", "rck_center"] ,
            'center_vel': self["halo", "rck_velocity"] ,
            'rvir': self["halo", "rck_rvir"] ,
            'vmax': self["halo", "rck_vmax"] ,
            'vrms': self["halo", "rck_vrms"]                           
        }
