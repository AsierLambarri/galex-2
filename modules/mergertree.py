...
class MergerTree:
    ...
    def load_halo(self, pdir, sub_tree, redshift=None, snapshot=None):
        ...
        ds = config.loader(fn)
        sp = ds.sphere((halo[["position_x", "position_y", "position_z"]].values[0].astype(float), 'kpccm'), (halo["virial_radius"].values.astype(float), 'kpccm') )
        
        stars = DataContainer(sp, "stars")
        dm = DataContainer(sp, "darkmatter")
        gas = DataContainer(sp, "gas")
        return [stars, dm, gas]
