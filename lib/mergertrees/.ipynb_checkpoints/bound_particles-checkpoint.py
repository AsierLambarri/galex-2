...
"""    if cm is None:
        cm = np.average(pos.in_units("kpc"), axis=0, weights=mass)
    else:
        pass
        
    if vcm is None:
        vcm = np.average(vel.in_units("km/s"), axis=0, weights=mass)        
    else:
        pass
    ...
    if ~refine:
        return bound_mask if ids is None else bound_mask, ids[bound_mask]
    else:
"""
